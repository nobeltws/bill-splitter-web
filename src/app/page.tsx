...
export default function UploadPage() {
	...
	return (
		<Wrapper>
			...
			<Subtitle>
				Upload a photo of your receipt and we'll split it for you
			</Subtitle>
			<ContentArea>
				<FileUpload
					file={file}
			...
			<SessionsSection>
				...
				<HintText>
					The number used when the session was created
				</HintText>
				...
				{sessions !== null && (
					<SessionList>
						{sessions.length === 0 ? (
							<EmptyState>
								No sessions found for this number.
							</EmptyState>
						) : (
							sessions.map((session) => (
								<SessionItem key={session.sessionId}>
									<SessionLink
										href={`/sessions/${session.sessionId}/dashboard`}
									>
										<SessionInfo>
											<SessionName>
												{session.name}
