# Task 6: Session Creation Page

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build the session creation page where hosts review parsed items, adjust charges, and create a bill-splitting session.

**Architecture:** Client component reading from ReceiptContext. Redirects to `/` if context is empty. On submit, calls `POST /api/sessions` and navigates to the host dashboard.
...
Navigate to `/sessions/new` directly — should redirect to `/` since context is empty.
